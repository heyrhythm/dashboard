...
const HEADER_COLOR = "#222B39";
const SUBTEXT_COLOR = "#868f9a";
const ACCENT = "#101317";
const FONT = "'Inter', 'Montserrat', 'Segoe UI', Arial, sans-serif";

const COLORS = [
  "#8884d8", "#82ca9d", "#ffc658", "#ff7300", "#00ff00", "#ff00ff", "#00ffff", "#ff0000", "#3949ab", "#d500f9",
  "#64dd17", "#ffd600", "#ff5252", "#00b8d4", "#a1887f", "#c51162", "#4e342e", "#1de9b6", "#607d8b",
];

// Register Chart.js components
Chart.register(CategoryScale, LinearScale, PointElement, BarElement, LineElement, ArcElement, Tooltip, ChartLegend);
...
const Dashboard = () => {
  ...
  const getUniqueValues = (field) =>
    [...new Set(rawData.map((item) => item[field]).filter(Boolean))].sort();
  ...
  const barChartData = {
    ...
    datasets: [
      { label: "Intensity", data: regionStats.map(d => d.intensity), backgroundColor: "#3949ab" },
      { label: "Relevance", data: regionStats.map(d => d.relevance), backgroundColor: "#101317" },
      { label: "Likelihood", data: regionStats.map(d => d.likelihood), backgroundColor: "#ff7300" },
    ],
  };
  ...
  const scatterDataArr = useMemo(
    () =>
      filteredData
        .filter(
          ({ intensity, likelihood }) => intensity != null && likelihood != null
        )
      ...
      <div className="dashboard-layout">
        ...
        <aside className="dashboard-sidebar">
          ...
          <div style={{ flex: 1, overflowY: "auto" }}>
            {[
              ["end_year", "End Year"], ["topic", "Topic"], ["sector", "Sector"], ["region", "Region"],
              ["pestle", "PESTLE"], ["source", "Source"], /*['swot', 'SWOT'],*/ ["country", "Country"], ["city", "City"],
            ].map(([field, label]) => (
              <div key={field} style={{ marginBottom: 15 }}>
                <label style={{
                  fontWeight: 600, fontSize: 15,
                  color: SUBTEXT_COLOR, marginBottom: 2,
                }}>{label}</label>
                <select
          ...
          <button
            ...
            style={{
              background: ACCENT, color: "#fff", fontWeight: 700, fontFamily: FONT,
              border: "none", borderRadius: 6, padding: "10px 22px", fontSize: 16,
              marginTop: 18, boxShadow: "0 2px 6px rgba(16,19,23,0.04)", cursor: "pointer", width: "100%",
            }}
          >Reset All</button>
        </aside>

        {/* Main dashboard area with responsive content and footer */}
        <main className="dashboard-main">
          <div className="dashboard-main-content">
            ...
            <ChartJsWrapper title="Region Metrics">
              <Bar
                ...
                options={{
                  responsive: true,
                  plugins: {
                    legend: { position: "top", labels: { color: SUBTEXT_COLOR, font: { family: FONT } } },
                  },
                  maintainAspectRatio: false,
                  scales: {
            ...
            <ChartJsWrapper title="Sector Distribution">
              <Pie
                ...
                options={{
                  responsive: true,
                  plugins: {
                    legend: { position: "right", labels: { color: SUBTEXT_COLOR, font: { family: FONT } } },
                  },
                  maintainAspectRatio: false,
                }}
                height={400}
            ...
            <section className="mt-5 px-3">
              <h3 className="mb-3"
                style={{ color: HEADER_COLOR, fontWeight: 700, fontSize: 22, fontFamily: FONT }}>
                Detailed Data View
              </h3>
                  ...
                  <tbody>
                    {filteredData.slice(0, 10).map((item, index) => (
                      <tr key={index} style={{ fontWeight: 500, color: ACCENT }}>
                        ...
                        <td>{item.sector}</td>
                        <td>
                          <span style={{ display: "inline-block", background: "#3949ab", color: "#fff", borderRadius: "4px", padding: "2px 10px", fontWeight: 600 }}>{item.intensity}</span>
                        </td>
                        <td>
                          <span style={{ display: "inline-block", background: "#101317", color: "#fff", borderRadius: "4px", padding: "2px 10px", fontWeight: 600 }}>{item.likelihood}</span>
                        </td>
                        <td>
                          <span style={{ display: "inline-block", background: "#ff7300", color: "#fff", borderRadius: "4px", padding: "2px 10px", fontWeight: 600 }}>{item.relevance}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
